...
<script>${rendererJs}</script>
  ...
  function buildformsJson(fullSchema) {
    if (!fullSchema || !Array.isArray(fullSchema.tabs)) return [];

    // Map FormCraft conditional operators 
    ...
    // Map FormCraft conditional actions
    ...
    // Helper: turn an options array (['US','UK','IN']) 
  ...
  // Reverse map for  operators → FormCraft operators
  ...
  //  field types → FormCraft type. Most pass through; a few are simplified.
  ...
  //  field definition (flat or nested) into a FormCraft field.
  function nfFieldToFcField(nf) {
    // Flatten: nested options take precedence over flat keys
  ...
  // After all fields are converted, replace `_whenName`
  ...
  function parseImportedSchema(parsed, fallbackName) {
    ...
    // Shape 2:  TabbedForm — array of tab definitions
    if (Array.isArray(parsed) && parsed.length && parsed[0].fields && Array.isArray(parsed[0].fields)) {
      ...
      // Resolve conditional `_whenName` markers to ids (per tab;  conditions are local)
      imported.tabs.forEach(t => resolveConditionalIds(t.fields));
      return imported;
    }

    // Shape 3:  flat — single array of field definitions
    ...
    throw new Error('Unrecognized schema format. Expected FormCraft schema,  tabbed array, or  flat field array.');
  }
  ...
  document.getElementById('saveBtn').addEventListener('click', async () => {
    ...
    const formsJson = buildformsJson(fullSchema);
    try {
